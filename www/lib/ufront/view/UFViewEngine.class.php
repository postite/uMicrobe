...
class ufront_view_UFViewEngine {
	...
	public function getTemplate($path, $templatingEngine = null) {
		...
		if($tmp) {
			...
			if($tmp1) {
				$tmp2 = new tink_core__Future_SyncFuture(new tink_core__Lazy_LazyConst(tink_core_Outcome::Success((property_exists($cached, "b") ? $cached->b: array($cached, "b")))));
				$GLOBALS['%s']->pop();
				return $tmp2;
			}
		}
		...
		{
			$tmp3 = tink_core__Future_Future_Impl_::_tryFailingMap($tplStrReady, array(new _hx_lambda(array(&$_gthis, &$finalPath, &$path, &$templatingEngine), "ufront_view_UFViewEngine_4"), 'execute'));
			$GLOBALS['%s']->pop();
			return $tmp3;
		}
		$GLOBALS['%s']->pop();
...
function ufront_view_UFViewEngine_4(&$_gthis, &$finalPath, &$path, &$templatingEngine, $tplStr) {
	{
		...
		try {
			...
			$e = $_ex_;
			{
				...
				{
					$tmp = tink_core_Outcome::Failure(tink_core_TypedError::withData(null, "Failed to parse template " . _hx_string_or_null($finalPath) . " using " . _hx_string_or_null($templatingEngine->type), $e, _hx_anonymous(array("fileName" => "UFViewEngine.hx", "lineNumber" => 205, "className" => "ufront.view.UFViewEngine", "methodName" => "getTemplate"))));
					$GLOBALS['%s']->pop();
					return $tmp;
				}
			}
