...
#end
class NumericStepper<T:ValueName> extends Atom<T> implements Microbe<T>{

	
	
	
	public function render():String{
		return '<input ${getId()} type="number" name="${data.n}" ${getClasses()} value="${data.v}">' ;
	}

	public function execute(ctx):Void{
		trace( "i'ma a numeric stepper");
		#if js
